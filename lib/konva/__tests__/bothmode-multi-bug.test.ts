...
import { getBothmodeEdgesWithRelativeLabels, getNormalizedDistances } from '../labelUtils';
...
// =====================================================================
describe('observation C (Phase H-3e 共通根 2): helper 関数 getNormalizedDistances 経由で distances key 不一致が解消される', () => {
  ...
  beforeAll(() => {
    ...
    // Phase H-3e (共通根 2、 案 2C'): AutoLayoutModal の normalizedDistances useMemo は
    // getNormalizedDistances(rawBuilding, normalizedBuilding, rawDistances) を呼ぶ。
    // テストでも同じ helper 関数経由で normalize してから cascade に渡す
    // (= 修正経路をテストが通るための必須条件)。
    const normalizedDistances2F = getNormalizedDistances(buildingRect2F, normalizedBuilding2F, distances2F);
    ...
    result = computeBothmode2FLayout(
      ...
      normalizedDistances2F, distances1F, scaffoldStart,
    );
  ...
  it('⭐ at SE 出辺 2A 1800×6=10800 候補: helper 関数経由で actualEndDistanceMm = 900 (= 仕様 b で 10800-9000-900) になる', () => {
  ...
  it('⭐ at SE 出辺 2A 1800×5+900+600+200=10700 候補: helper 関数経由で actualEndDistanceMm = 800 になる', () => {
  ...
  it('⭐ at SE 出辺 2A: helper 関数経由で desiredEndDistanceMm = 888 (= 西の入力値、 仕様 b) になる', () => {
